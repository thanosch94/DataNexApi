...
namespace DataNexApi.Controllers
{
    ...
    public class ShippingMethodsController : BaseController
    {
        ...
        public ShippingMethodsController(ApplicationDbContext context, IMapper mapper) : base(context)
        {
            _context = context;
            _mapper = mapper;
        }
        ...
        public async Task<IActionResult> GetAll()
        {
            ...
            var data = await _context.ShippingMethods.Where(x => x.CompanyId == companyId).ToListAsync();
        ...
        public async Task<IActionResult> GetLookup()
        {
            ...
            var data = await _context.ShippingMethods.Where(x => x.CompanyId == companyId).Select(x => new ShippingMethodDto()
            {
                Id = x.Id,
                Name = x.Name,
            }).ToListAsync();
        ...
        public async Task<IActionResult> GetById(Guid id)
        {
            ...
            var data = await _context.ShippingMethods.FirstOrDefaultAsync(x => x.Id == id && x.CompanyId == companyId);
            var dto = _mapper.Map<ShippingMethodDto>(data);

            return Ok(dto);
        }

        [HttpPost("insertdto")]
        public async Task<IActionResult> InsertDto([FromBody] ShippingMethodDto dto)
        {
            Guid companyId = GetCompanyFromHeader();

            var actionUser = await GetActionUser();

            var data = new ShippingMethod();

            var exists = await _context.ShippingMethods.Where(x => x.Name == dto.Name && x.CompanyId == companyId).FirstOrDefaultAsync();
            if (exists == null)
            {
                data.Name = dto.Name;
                ...
                lock (_lockObject)
                {
                    var maxNumber = _context.ShippingMethods.Where(x => x.CompanyId == companyId).Max(x => (x.SerialNumber)) ?? 0;
                    data.SerialNumber = maxNumber + 1;
                    data.Code = data.SerialNumber.ToString().PadLeft(5, '0');

                    try
                    {
                        _context.ShippingMethods.Add(data);
            ...
            {
                return BadRequest("Shipping Method already exists");
            }

        }

        [HttpPut("updatedto")]
        public async Task<IActionResult> UpdateDto([FromBody] ShippingMethodDto dto)
        {
            Guid companyId = GetCompanyFromHeader();

            var actionUser = await GetActionUser();

            var data = await _context.ShippingMethods.Where(x => x.Id == dto.Id && x.CompanyId == companyId).FirstOrDefaultAsync();
            ...
            var dtoData = _mapper.Map<ShippingMethodDto>(data);
        ...
        public async Task<IActionResult> DeleteById(Guid id)
        {
            ...
            _context.ShippingMethods.Remove(data);
